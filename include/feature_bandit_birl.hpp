...
#ifndef feature_bandit_birl_h
#define feature_bandit_birl_h
...
class FeatureBanditBIRL { // BIRL process
   ...
   protected:
      ...
      void modifyFeatureWeightsRandomly(FeatureMDP * gmdp, double step_size);
      void sampleL1UnitBallRandomly(FeatureMDP * gmdp);
      void updownL1UnitBallWalk(FeatureMDP * gmdp, double step);
      void manifoldL1UnitBallWalk(FeatureMDP * gmdp, double step, int num_steps);
      void manifoldL1UnitBallWalkAllSteps(FeatureMDP * gmdp, double step);


      double* posteriors = nullptr;
      FeatureMDP* MAPmdp = nullptr;
      double MAPposterior;
      
   public:
   
     FeatureMDP* mdp = nullptr; //original MDP 
     FeatureMDP** R_chain = nullptr; //storing the rewards along the way
     ...
     FeatureBanditBIRL(BanditMDP* init_mdp, double min_reward, double max_reward, unsigned int chain_len, double step, double conf, int samp_flag=0, bool reject=false, int num_step=1, int post_flag=0, double tol = 0.0001, unsigned int _horizon=1, int _seed = 321):  
     r_min(min_reward), r_max(max_reward), step_size(step), chain_length(chain_len), alpha(conf), sampling_flag(samp_flag), mcmc_reject(reject), num_steps(num_step), posterior_flag(post_flag), numTolerance(tol), horizon(_horizon), seed(_seed){ 
     
        unsigned int numActions = init_mdp -> getNumActions();
        ...
        //copy init_mdp
        mdp = new BanditMDP(numActions, nfeatures, fweights, sfeatures);
        initializeMDP(); //set weights to (r_min+r_max)/2
        
        
        MAPmdp = new BanditMDP(numActions, nfeatures, fweights, sfeatures);
        
        MAPmdp->setFeatureWeights(mdp->getFeatureWeights());
        MAPposterior = 0;
        
        R_chain = new FeatureMDP*[chain_length];
        posteriors = new double[chain_length];    
        iteration = 0;
        
       }; 
       
      FeatureMDP* getMAPmdp(){return MAPmdp;}
      ...
      FeatureMDP** getRewardChain(){ return R_chain; };
      FeatureMDP* getMeanMDP(int burn, int skip);
      double* getPosteriorChain(){ return posteriors; };
      FeatureMDP* getMDP(){ return mdp;};
      double calculateBIOPosterior(FeatureMDP* gmdp);
      double logsumexp(double* nums, unsigned int size);
      double calculateLogSoftmax(FeatureMDP* gmdp);
      bool isDemonstration(pair<double,double> s_a);
      double getNumTolerance(){return numTolerance;};
           
};

void FeatureBanditBIRL::run()
{
    runBirl = true;   
     //cout.precision(10);
    ...
    //cout << "testing" << endl;
    mdp->valueIteration(numTolerance);//deterministicPolicyIteration(policy);
    ...
    for(unsigned int itr=1; itr < chain_length; itr++)
    {
      ...
      cout << "==============================" << endl;
      cout << "itr: " << itr << endl;
      FeatureMDP* temp_mdp = new BanditMDP (mdp->getNumActions(), mdp->getNumFeatures(), mdp->getFeatureWeights(), mdp->getStateFeatures());
      ...
      temp_mdp->valueIteration(numTolerance, mdp->getValues());
      ...
      cout << "new posterior: " << exp(new_posterior) << endl;
      cout << "old posterior: " << exp(posterior) << endl;
      ...
      if ( r < probability ) //policy_changed && 
      {
        ...
        delete temp_mdp;
         ...
         if(mcmc_reject)
         {
            ...
            //make a copy of mdp
            FeatureMDP* mdp_copy = new BanditMDP (mdp->getNumActions(), mdp->getNumFeatures(), mdp->getFeatureWeights(), mdp->getStateFeatures());
...
double FeatureBanditBIRL::logsumexp(double* nums, unsigned int size) {
...
/*//try 1
double BanditBIRL::calculatePosterior(BanditMDP* gmdp) //assuming uniform prior
{
    cout << "demos" << endl;
    displayDemos();
...
double FeatureBanditBIRL::calculateBIOPosterior(FeatureMDP* gmdp) //assuming uniform prior
{
  ...
  unsigned int numDemos = demo.size() / horizon;  //TODO: hack for now
  ...
  vector<vector<pair<unsigned int, unsigned int > > > optTeachingSoln =solveSetCoverOptimalTeaching(gmdp, K, horizon);
  ...
  likelihood = exp(- 10*abs(softMatch_acutal - softMatch_optimal));
...
//normal likelihood 
double FeatureBanditBIRL::calculateLogSoftmax(FeatureMDP* gmdp) //assuming uniform prior
{
...
void FeatureBanditBIRL::modifyFeatureWeightsRandomly(FeatureMDP * gmdp, double step)
{
...
void FeatureBanditBIRL::sampleL1UnitBallRandomly(FeatureMDP * gmdp)
{
...
void FeatureBanditBIRL::updownL1UnitBallWalk(FeatureMDP * gmdp, double step)
{
   unsigned int numFeatures = gmdp->getNumFeatures();
...
void FeatureBanditBIRL::manifoldL1UnitBallWalk(FeatureMDP * gmdp, double step, int num_steps)
{
   unsigned int numFeatures = gmdp->getNumFeatures();
...
void FeatureBanditBIRL::manifoldL1UnitBallWalkAllSteps(FeatureMDP * gmdp, double step)
{
   unsigned int numFeatures = gmdp->getNumFeatures();
...
void FeatureBanditBIRL::addPositiveDemos(vector<pair<unsigned int,unsigned int> > demos)
{
...
void FeatureBanditBIRL::addNegativeDemos(vector<pair<unsigned int,unsigned int> > demos)
{
...
void FeatureBanditBIRL::displayDemos()
{
   displayPositiveDemos();
   displayNegativeDemos();
}
      
void FeatureBanditBIRL::displayPositiveDemos()
{
...
void FeatureBanditBIRL::displayNegativeDemos()
{
...
void FeatureBanditBIRL::initializeMDP()
{
   if(sampling_flag == 0)
   {
...
bool FeatureBanditBIRL::isDemonstration(pair<double,double> s_a)
{
     for(unsigned int i=0; i < positive_demonstration.size(); i++)
   {
...
FeatureMDP* FeatureBanditBIRL::getMeanMDP(int burn, int skip)
{
...
//    //create new MDP with average weights as features
    FeatureMDP* mean_mdp = new BanditMDP(MAPmdp->getNumActions(), MAPmdp->getNumFeatures(), aveWeights, MAPmdp->getStateFeatures());
