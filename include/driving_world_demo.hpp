...
enum Bottom_Msg {NO_MSG, SPACE, SPACE_REPEAT, ANY_KEY};
...
//world attributes
        ...
        const int NUM_ONROAD_LANES = 3;
        const int NUM_ACTIONS = 3;

        const int SCREEN_WIDTH = 800;
        const int SCREEN_HEIGHT = 360;  //6 car widths
        ...
        SDL_Surface *splash_message = NULL;
	SDL_Surface *bottom_message1 = NULL;
	SDL_Surface *bottom_message2 = NULL;

        SDL_Surface *screen = NULL;

        //The font
        TTF_Font *font = NULL;
        TTF_Font *splash_font = NULL;
	TTF_Font *bottom_font = NULL;

        //The color of the font
        SDL_Color blackColor = { 1, 0, 0 };
        SDL_Color whiteColor = { 1, 0, 1 };
...
class DrivingWorld 
{
    private:
        ...
        void updateFrame(string display_msg, string splash_msg, Bottom_Msg msg);
        bool init();
    ...
    public:
        ...
        void setSplashScreen(string splash_msg)
        {
             if( init() == false )
              {
           ...
           // }
            ...
            updateFrame(" ", splash_msg, ANY_KEY);
        ...
        void setSplashScreen(string splash_msg, int seed)
        {
             if( init() == false )
              {
                cout << "can't init" << endl;
        ...
        //Initializes them to random positions
        void initializeCarPositions(int seed);
        ...
        vector<pair<string,unsigned int> > startHumanDemo(int demo_length, bool repeatable);
        vector<pair<string,unsigned int> > startHumanDemo(int demo_length, bool repeatable, int seed);
...
void DrivingWorld::initializeCarPositions(int seed)
{
  //cout << "seed = " << seed << endl;
  srand(seed);
  //Initialize the offsets
  //agent car starts in lane 2
  int myCarLane = rand() % NUM_LANES;
  
  box.x = myCarLane*LANE_WIDTH + LANE_WIDTH/2 - CAR_WIDTH/2;
  box.y = 5 * CAR_HEIGHT;


  //start in lane 1
  int bad_car_lane = 1 + rand() % NUM_ONROAD_LANES ;  //keep other cars on road
  //cout << "bad car lane 1 = " << bad_car_lane << endl;
  bad_car_box.x = bad_car_lane*LANE_WIDTH + LANE_WIDTH/2 - CAR_WIDTH/2;
  bad_car_box.y = 4*CAR_HEIGHT;

  //pick random lane such that not same as badcar
  int bad_car2_lane = 0;
  do 
  {
    bad_car2_lane = 1 + rand() % NUM_ONROAD_LANES;  //keep other cars on road
    //start in lane 3
    bad_car_box2.x = bad_car2_lane*LANE_WIDTH + LANE_WIDTH/2 - CAR_WIDTH/2;
    bad_car_box2.y = 0;
  } while((getCurrentLaneNumber(1) == getCurrentLaneNumber(2)));
  //cout << "bad car lane 2 = " << bad_car2_lane << endl;
  //Set the car's dimentions
  box.w = CAR_WIDTH;
  box.h = CAR_HEIGHT;
  bad_car_box.w = CAR_WIDTH;
  ...
  //perform a random number of NoOps to get into random configuration
  int numNoOps = rand() % 30;
  //cout << "Moving for " << numNoOps << " steps" << endl;
  for(int step = 0; step < numNoOps; step++)
  {
    move();
  ...
  // cout << "my lane: " << getCurrentLaneNumber(0) << ", my position: " << box.x << "," << box.y << endl;
  // cout << "car 1 lane: " << getCurrentLaneNumber(1) << ", my position: " << bad_car_box.x << "," << bad_car_box.y << endl;
  // cout << "car 2 lane: " << getCurrentLaneNumber(2) << ", my position: " << bad_car_box2.x << "," << bad_car_box2.y << endl;
  // cout << "**************" << endl;

  //TODO: need better way to detect collision
  // if(isCollision())
  // {
  //   cout << "!!!!!!!!!!!!!!!!!!!!!!!!!!!" << endl;
  //   cout << "COLLISION" << endl;
...
void DrivingWorld::move()
{
  ...
  while((bad_car_box.y > SCREEN_HEIGHT)  || (two_cars && (getCurrentLaneNumber(1) == getCurrentLaneNumber(2)))){
    bad_car_box.y = 0;
  ...
  if(two_cars)
  {
      while( bad_car_box2.y > SCREEN_HEIGHT || (getCurrentLaneNumber(1) == getCurrentLaneNumber(2))){
        bad_car_box2.y = 0;
...
vector<pair<string,unsigned int> > DrivingWorld::startHumanDemo(int demo_length, bool repeatable)
{
...
//    {
  ...
  while( quit == false && steps < demo_length)
  {
    ...
    if(!quit)
    {
        ...
        if(curState.getRewardFeature(0) == 1)
        {
            display_msg = "Collision!";
	    ...
	    //cout << display_msg << endl;
        
        
	if(repeatable)
	        updateFrame(display_msg, " ", SPACE_REPEAT); 
	else
	        updateFrame(display_msg, " ", SPACE); 
	//Update the screen
        if( SDL_Flip( screen ) == -1 )
        {
          cout << "can't update screen" << endl;
  ...
  {
    cout << "redo" << endl;
    //remove demos to signal a redo of the demonstration
    demonstration.clear();
    return demonstration;
  }
}
...
//reset game and return starting state
vector<pair<string,unsigned int> > DrivingWorld::startHumanDemo(int demo_length, bool repeatable, int seed)
{
...
//    {
  ...
  while( quit == false && steps < demo_length)
  {
    ...
    if(!quit)
    {
        ...
        string display_msg;
        curState = getCurrentState();
        cout << "current state: " << curState.toStateString() << endl;
        if(curState.getRewardFeature(0) == 1)
        {
            display_msg = "Collision!";
...
//reset game and return starting state
State DrivingWorld::startNewEpoch(int seed)
{
    //reset score
    score = 0;
...
//  cout << "Next lane: " << lane << endl;
    ...
    if(visualize)
    {
            ...
            //cout << display_msg << endl;
        updateFrame(display_msg, " ", NO_MSG);   
        //Update the screen
        if( SDL_Flip( screen ) == -1 )
        {
          cout << "can't update screen" << endl;
...
int DrivingWorld::handle_input()
{
  ...
  if( event.type == SDL_KEYDOWN )
  {
    ...
    switch( event.key.keysym.sym )
    {
      ...
      case SDLK_SPACE: action = -100; break;
      case SDLK_ESCAPE: action = -200; break;
      default: break;
    }
  }
  return action;
...
void DrivingWorld::updateFrame(string display_msg, string splash_msg, Bottom_Msg bottom_msg)
{
 ...
 //Fill the screen white
        ...
        //update the message
        message = TTF_RenderText_Solid( font, display_msg.c_str(), blackColor );
        splash_message = TTF_RenderText_Solid( splash_font, splash_msg.c_str(), blackColor );
        //Show the message
        apply_surface( LANE_WIDTH * 5 + 30, message->h, message, screen );
        apply_surface( LANE_WIDTH * 5 + 30, splash_message->h, splash_message, screen );

	string bottom_string2;
	string bottom_string1;
	if(bottom_msg == NO_MSG)
	{
		//don't put anything on bottom
		bottom_string1 = " ";
		bottom_string2 = " ";
	}
	else if(bottom_msg == SPACE)
	{
		bottom_string1 = " ";
		bottom_string2 = "<SPACE> to stop";
		
	}
	else if(bottom_msg == SPACE_REPEAT)
	{
		bottom_string1 = "<ESC> to redo";
		bottom_string2 = "<SPACE> to stop";
	}
	else if(bottom_msg == ANY_KEY)
	{
		bottom_string1 = "Press any key";
		bottom_string2 = "";
	}
	bottom_message1 = TTF_RenderText_Solid( bottom_font, bottom_string1.c_str(), blackColor );
	apply_surface( LANE_WIDTH * 5 + 30, 300, bottom_message1, screen );
	bottom_message2 = TTF_RenderText_Solid( bottom_font, bottom_string2.c_str(), blackColor );
	apply_surface( LANE_WIDTH * 5 + 30, 400, bottom_message2, screen );
...
bool DrivingWorld::load_files()
{
  ...
  splash_font = TTF_OpenFont( "Times_New_Roman_Bold.ttf", 40 ); 
  bottom_font = TTF_OpenFont( "Times_New_Roman_Bold.ttf", 30 );
